...
@DisplayName("Adjacency Detector Tests")
class AdjacencyDetectorTests {

    @Nested
    @DisplayName("Adjacency Conversion Logic Tests")
    class ConversionLogicTests {
        ...
        @Description("Validates that an empty list of CSV data rows results in an empty list of properties.")
        @Severity(SeverityLevel.MINOR)
        void convertToProperties_emptyData() {
            List<String[]> data = new ArrayList<>();
            List<PropertyPolygon> properties = AdjacencyDetector.convertToProperties(data);
        ...
        @Test
        @DisplayName("Skip properties with 'NA' in specific fields")
        @Description("Ensures rows with 'NA' in Freguesia, Municipio, or Ilha are skipped.")
        @Severity(SeverityLevel.MINOR)
        void convertToProperties_skipsNAFields() {
            List<String[]> data = new ArrayList<>();
            data.add(new String[]{"ID", "V1", "V2", "V3", "V4", "Owner", "Freguesia", "Municipio", "Ilha"});
            data.add(new String[]{"1", "0,0", "1,0", "1,1", "0,1", "OwnerA", "NA", "City1", "Island1"}); // NA in Freguesia
            data.add(new String[]{"2", "0,0", "1,0", "1,1", "0,1", "OwnerB", "Parish1", "NA", "Island1"}); // NA in Municipio
            data.add(new String[]{"3", "0,0", "1,0", "1,1", "0,1", "OwnerC", "Parish1", "City1", "NA"}); // NA in Ilha
            List<PropertyPolygon> result = AdjacencyDetector.convertToProperties(data);
            assertTrue(result.isEmpty(), "Properties with 'NA' in Freguesia, Municipio, or Ilha should be skipped.");
        }

        @Test
        @DisplayName("Skip properties with polygons without vertices")
        @Description("Ensures rows with polygons that have no vertices are skipped.")
        @Severity(SeverityLevel.MINOR)
        void convertToProperties_skipsNoVertices() {
            List<String[]> data = new ArrayList<>();
            data.add(new String[]{"ID", "V1", "V2", "V3", "V4", "Owner", "Freguesia", "Municipio", "Ilha"});
            data.add(new String[]{"1", "", "", "", "", "OwnerA", "Parish1", "City1", "Island1"}); // No vertices
            List<PropertyPolygon> result = AdjacencyDetector.convertToProperties(data);
            assertTrue(result.isEmpty(), "Properties with no vertices should be skipped.");
        }

        @Test
        @DisplayName("Skip properties with missing owner")
        @Description("Ensures rows with missing owner values are skipped.")
        @Severity(SeverityLevel.MINOR)
        void convertToProperties_skipsMissingOwner() {
            List<String[]> data = new ArrayList<>();
            data.add(new String[]{"ID", "V1", "V2", "V3", "V4", "Owner", "Freguesia", "Municipio", "Ilha"});
            data.add(new String[]{"1", "0,0", "1,0", "1,1", "0,1", "", "Parish1", "City1", "Island1"}); // Missing owner
            List<PropertyPolygon> result = AdjacencyDetector.convertToProperties(data);
            assertTrue(result.isEmpty(), "Properties with missing owner should be skipped.");
        }
        ...
        @Test
        @DisplayName("Skip null property")
        @Description("Ensures rows with null properties are skipped.")
        @Severity(SeverityLevel.MINOR)
        void convertToProperties_skipsNullProperty() {
            List<String[]> data = new ArrayList<>();
            data.add(new String[]{"ID", "V1", "V2", "V3", "V4", "Owner", "Freguesia", "Municipio", "Ilha"});
            data.add(null); // Null row
            List<PropertyPolygon> result = AdjacencyDetector.convertToProperties(data);
            assertTrue(result.isEmpty(), "Null properties should be skipped.");
        }
    }

    @Nested
    @DisplayName("Adjacency Detection Tests")
    class AdjacencyDetectionTests {
    ...
    @Nested
    @DisplayName("Adjacency Validation Tests")
    ...
    @Nested
    @DisplayName("Adjacency Vertex Sharing Tests")
    class VertexSharingTests {
    ...
    @Nested
    @DisplayName("Valid Adjacent Pairs Tests")
    class AdjacentPairsTests {
